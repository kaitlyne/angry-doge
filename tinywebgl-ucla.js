...
var shapes_in_use = [], shaders_in_use = [], textures_in_use = [], active_shader, texture_filenames_to_load = [], gl, g_addrs;
// ****** GLOBAL VARIABLES *******
...
function Declare_Any_Class(name, methods, superclass=Object, scope=window) // Making javascript behave more like Object Oriented C++
{
    ...
    // When new gets called on the class name, call its construct():
    var p = Object.create(superclass.prototype);
    // Use prototypes to make the object.  Begin with the superclass's.
    p.constructor = scope[name];
    // Override the prototype so that member functions work.
    p.class_name = name;
    // Let classes be aware of their type name.
    p.define_data_members = function(args) {
        for (let i in args)
            this[i] = args[i];
    }
    // Optional utility method for all classes
    for (let i in methods)
        p[i] = methods[i];
    // Include all the other supplied methods.
    scope[name].prototype = p;
    // The class and its prototype are complete.
}
...
Declare_Any_Class("Shape", {
    'construct': function(args) {
        ...
        // Immediately fill in appropriate vertices via polymorphism, calling whichever sub-class's populate().
    },
    'insert_transformed_copy_into': function(recipient, args, points_transform=mat4() ) {
        var temp_shape = new (window[this.class_name])(...args);
        ...
        for (var i = 0; i < temp_shape.indices.length; i++)
            recipient.indices.push(temp_shape.indices[i] + recipient.positions.length);
    ...
    'draw': function(graphics_state, model_transform, material) // The same draw() function is used for every shape -
    {
        ...
        active_shader.update_uniforms(graphics_state, model_transform, material);
        ...
        if (material.texture_filename) // Omit the texture string parameter from Material's constructor to signal not to draw a texture.
        {
            ...
            if (textures_in_use[material.texture_filename])
                gl.bindTexture(gl.TEXTURE_2D, textures_in_use[material.texture_filename].id);
        } else {
            gl.uniform1f(g_addrs.USE_TEXTURE_loc, 0);
            g_addrs.shader_attributes[2].enabled = false;
        }

        for (var i = 0, it = g_addrs.shader_attributes[0]; i < g_addrs.shader_attributes.length,
        it = g_addrs.shader_attributes[i]; i++)
            if (it.enabled) {
        ...
        // If no indices were provided, assume the vertices are arranged in triples
    },
    'auto_flat_shaded_version': function(args) {
        var populate_and_auto_flat_shade = (function(superclass) {
            return function(args) {
                superclass.prototype.populate.apply(this, arguments);
                this.duplicate_the_shared_vertices();
                this.flat_shade();
            }
        })(window[this.class_name]);
        Declare_Any_Class(this.class_name.concat("_flat"), {
            "populate": populate_and_auto_flat_shade
        }, window[this.class_name]);
    ...
    'duplicate_the_shared_vertices': function(offset=0, index_offset=0) {
        ...
        var temp_positions = this.positions.slice(0, offset)
          , temp_tex_coords = this.texture_coords.slice(0, offset)
          , temp_normals = this.normals.slice(0, offset);
        var temp_indices = this.indices.slice(0, index_offset);

        for (var counter = index_offset; counter < this.indices.length; counter++) {
            temp_positions.push(this.positions[this.indices[counter]]);
            temp_tex_coords.push(this.texture_coords[this.indices[counter]]);
    ...
    'flat_shade': function(index_offset=0) {
        ...
        for (var counter = index_offset; counter < this.indices.length; counter += 3) // Iterate through triangles (every triple in the "indices" array)
        {
            ...
            var n1 = normalize(cross(subtract(p1, p2), subtract(p3, p1)));
            // Cross two edge vectors of this triangle together to get the normal

            if (length(add(scale_vec(.1, n1), p1)) < length(p1))
                n1 = scale_vec(-1, n1);
            // Flip the normal if adding it to the triangle brings it closer to the origin.

            this.normals[indices[0]] = this.normals[indices[1]] = this.normals[indices[2]] = vec3(n1[0], n1[1], n1[2]);
            // Propagate the normal to the 3 vertices.
        }
    }
});
    ...
    'add': function(shortcut_combination, recipient=window, callback, opt) {
        ...
        var onkeypress = function(e) // On each keypress, this gets called [# of bound keys] times
        {
            ...
            for (let k of keycombo) // Check if current keycombo in consideration matches the actual keypress
            {
                ...
                if (k.length > 1 && special_keys[k] == code)
                    num_pressed++;
            ...
            if (num_pressed == keycombo.length && modifiers.ctrl.pressed == modifiers.ctrl.wanted && modifiers.shift.pressed == modifiers.shift.wanted && modifiers.alt.pressed == modifiers.alt.wanted && modifiers.meta.pressed == modifiers.meta.wanted) {
                callback.call(recipient, e);
                // *** Fire off the function that matched the pressed keys ***********************************
                if (!opt['propagate']) {
                    e.cancelBubble = true;
                    e.returnValue = false;
        ...
        if (ele.addEventListener)
            ele.addEventListener(opt['type'], onkeypress, false);
        else if (ele.attachEvent)
            ele.attachEvent('on' + opt['type'], onkeypress);
        else
            ele['on' + opt['type']] = onkeypress;
    },
...
Declare_Any_Class("Graphics_Addresses", // Find out the memory addresses internal to the graphics card of each of its variables, and store them here locally for the Javascript to use
{
    'construct': function(program) {
        ...
        new Shader_Attribute(gl.getAttribLocation(program, "vTexCoord"),2,gl.FLOAT,false,false,0,0), new Shader_Attribute(gl.getAttribLocation(program, "vColor"),3,gl.FLOAT,false,false,0,0)];

        var num_uniforms = gl.getProgramParameter(program, gl.ACTIVE_UNIFORMS);
        for (var i = 0; i < num_uniforms; ++i) {
            var u = gl.getActiveUniform(program, i).name.split('[')[0];
...
Declare_Any_Class("Shader", // Shader superclass.  Instantiate subclasses that have all methods filled in.  That loads a new vertex & fragment shader onto the GPU.
{
    ...
    'activate': function() {
        ...
        g_addrs = new Graphics_Addresses(this.program);
        active_shader = this;
    },
...
Declare_Any_Class("Canvas_Manager", // This class performs all the setup of doing graphics.   It informs OpenGL of which
{
    'construct': function(canvas_id, background_color) // functions to call during events - such as a key getting pressed or it being time to redraw.
    {
        ...
        for (let name of ["webgl", "experimental-webgl", "webkit-3d", "moz-webgl"])
            if (this.gl = gl = canvas.getContext(name))
                break;
        // Get the GPU ready, creating a new WebGL context for this canvas
        if (!gl && canvas.parentNode)
            canvas.parentNode.innerHTML = "Canvas failed to make a WebGL context.";
        ...
        this.controls = new Shortcut_Manager();
    ...
    'register_display_object': function(object) {
        this.displayables.unshift(object);
        object.init_keys(this.controls);
    },
    'render': function(time=0) // Animate shapes based upon how much measured real time has transpired.
    {
        ...
        for (let name in shapes_in_use)
            if (!shapes_in_use[name].sent_to_GPU)
                shapes_in_use[name].copy_onto_graphics_card();

        gl = this.gl;
        // Set the global gl variable to the current one that is drawing, belonging to this canvas.
        gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);
        // Clear its pixels and z-buffer.           
        for (var i = 0; i < this.displayables.length; i++) {
            this.displayables[i].display(time);
            // Draw each registered displayable.
            this.displayables[i].update_strings(this.shared_scratchpad);
        }
        window.requestAnimFrame(this.render.bind(this));
        // Now that this frame is drawn, request that it happen again as soon as all other OpenGL events are processed.
    }
});
...
Declare_Any_Class("Texture", // Wrap a pointer to a new texture buffer along with a new HTML image object.
{
    construct: function(filename, bool_mipMap, bool_will_copy_to_GPU=true) {
        ...
        // A single red pixel, as a placeholder image to prevent console warning
