  ...
  Declare_Any_Class("Surface_Of_Revolution", {
      populate: function(rows, columns, points, total_curvature_angle = 360, texture_coord_range = [
          [0, 1][0, 1]
      ]) {
          for (var i = 0; i <= rows; i++) // Travel down the curve spelled out by the parameter "points"
          {
              var frac = i / rows * (points.length - 1),
                  alpha = frac - Math.floor(frac), // Which points in that array are we between?
              ...
              for (var j = 0; j <= columns; j++) {
                  ...
                  this.positions.push(mult_vec(spin, currPoint).slice(0, 3));
                  this.normals.push(mult_vec(spin, normal).slice(0, 3));
  ...
  Declare_Any_Class("Rounded_Capped_Cylinder", // An alternative without three separate sections
      {
          populate: function(rows, columns) {
              Surface_Of_Revolution.prototype.insert_transformed_copy_into(this, [rows, columns, [vec3(0, 0, .5), vec3(1, 0, .5), vec3(1, 0, -.5), vec3(0, 0, -.5)]]);
          }
      }, Shape)
